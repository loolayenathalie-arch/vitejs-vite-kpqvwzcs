...
══════════════════════════════════════════════════════════ */
const T = {
  /* ── 1. ROSE — Étagère Boubou (douceurs) ── */
  rose: {
    main:    "#D4537E",   // primaire — boutons, badges
    light:   "#F4B8C8",  // bordures, accents
    bg:      "#FFF0F4",  // fond de panneau
    soft:    "#FFE0EC",  // post-its
    text:    "#72243E",  // texte sur fond clair
    dark:    "#8B1A40",  // texte sur fond coloré
  },
  /* ── 2. VIOLET — Étagère Cloud (pensées) ── */
  violet: {
    main:    "#7F77DD",
    light:   "#C8B8F0",
    bg:      "#F4EEFF",
    ...
    text:    "#3C3489",
    dark:    "#1E1860",
    glow:    "#AFA9EC",  // étoiles nuit, reflets
    night:   "#534AB7",  // mode nuit
  },
  /* ── 3. BLEU MER — Étagère Trash (vagues) ── */
  mer: {
    main:    "#1A6AAA",
    light:   "#90C4E8",
    bg:      "#E8F4FF",
    ...
    text:    "#0C3060",
    dark:    "#0A2448",
    sky:     "#3AB0E8",  // ciel marin
  },
  /* ── 4. BLEU NUIT — Étagère Lettre ── */
  nuit: {
    main:    "#1A3A78",
    light:   "#8098C8",
    bg:      "#E8EEF8",
    ...
    text:    "#0C1E50",
    dark:    "#060C28",
    deep:    "#040614",  // fond écran sommeil
    purple:  "#080720",  // gradient nuit
  },
  /* ── 5. VERT EAU — Étagère Bonheur / accueil ── */
  vert: {
    main:    "#1D9E75",
    light:   "#9FE1CB",
    bg:      "#E8F5EB",
    ...
    text:    "#085041",
    dark:    "#043828",
    pale:    "#A8E8D8",  // blobs déco
    bright:  "#5DCAA5",  // accents vifs
  },
  /* ── 6. DORÉ/SOLEIL — Étagère Bonheur ── */
  or: {
    main:    "#EF9F27",
    light:   "#FAC775",
    bg:      "#FFF8E6",
    ...
    text:    "#633806",
    dark:    "#3A1E00",
    warm:    "#D88A18",  // gradient bouton
  },
  /* ── NEUTRES ── */
  neutral: {
    white:   "#FFFFFF",
    cream:   "#FDF8F1",   // fond app jour
    beige:   "#F5E8CF",   // fond dégradé
    sand:    "#EAD5B0",   // fond dégradé mid
    tan:     "#DEC89A",   // fond dégradé dark
    brown:   "#3B2E22",   // texte principal
    brownMid:"#7A6A60",   // texte secondaire
    brownLight:"#B4A89E", // texte tertiaire
    wood:    "#8B5A28",   // bois bibliothèque
    woodDark:"#5A3418",   // bois sombre
    woodLight:"#D4A870",  // bois clair
  },
  /* ── SYSTÈME ── */
  sys: {
    error:    "#F0997B",
    ...
    warning:  "#EF9F27",
    info:     "#378ADD",
    overlay:  "rgba(30,20,10,0.75)",
    glass:    "rgba(255,255,255,0.85)",
    glassDark:"rgba(20,16,50,0.92)",
  },
};
...
/* ══════════════════════════════════════════════════════════
   CONFIGURATION VOIX
   ─────────────────────────────────────────────────────────
   Option A — ElevenLabs (meilleure qualité)
     → elevenlabs.io → API Keys → coller ici
   Option B — OpenAI TTS (très naturelle, voix nova/shimmer)
     → platform.openai.com → API Keys → coller ici
   ─────────────────────────────────────────────────────────
   Si les deux sont vides → Web Speech API (voix navigateur)
══════════════════════════════════════════════════════════ */
const ELEVENLABS_API_KEY  = ""; // ← sk_...
const ELEVENLABS_VOICE_ID = "XB0fDUnXU5powFXDhCwa"; // Charlotte
const OPENAI_API_KEY      = ""; // ← sk-...  (OpenAI)
const OPENAI_VOICE        = "nova"; // nova | shimmer | alloy | echo | fable | onyx
...
function useSpeech() {
  ...
  const speakOpenAI = useCallback(async (text, opts = {}) => {
    ...
    if (!url) {
      try {
        ...
        console.warn("OpenAI TTS → fallback:", err.message);
        setIsLoading(false);
        speakWebSpeech(text, opts);
        return;
      }
  ...
  const speakWebSpeech = useCallback((text, opts = {}) => {
    ...
    const speakPart = () => {
      ...
      u.onerror = () => { clearTimeout(watchdog); setTimeout(speakPart, 200); };
...
function PostIt({ item, color, onRemove, index, onSpeak, parentReply }) {
  ...
  return (
    <motion.div layout
      ...
      whileHover={{ scale:1.10, rotate:0, zIndex:20, boxShadow:"3px 6px 14px rgba(0,0,0,0.18)" }}
      style={{
        background: drawing
          ? color.postit
        ...
        position:"relative", lineHeight:1.5, wordBreak:"break-word",
            ...
            {parentReply && (
              <motion.div initial={{ opacity:0, y:4 }} animate={{ opacity:1, y:0 }}
                style={{
                  ...
                  fontSize:11, color:"#633806", lineHeight:1.4,
                }}>
                💛 {parentReply.text}
              </motion.div>
            )}
      ...
      <button onClick={onRemove} aria-label="Supprimer cette pensée"
        style={{
          ...
          cursor:"pointer", minHeight:44, fontWeight:800, lineHeight:1,
...
/* ══════════════════════════════════════════════════════════
   MINI CANVAS DESSIN
══════════════════════════════════════════════════════════ */
const DRAW_COLORS = ["#D4537E","#378ADD","#EF9F27","#1D9E75","#7F77DD","#D85A30","#3B2E22","#fff"];
...
function MiniCanvas({ color, onSave }) {
  ...
  const [brushColor, setBrushColor] = useState(color || "#D4537E");
  ...
  useEffect(() => {
    ...
    ctx.fillStyle = "#FFF9F0"; ctx.fillRect(0, 0, cv.width, cv.height);
  }, []);
  ...
  const draw = (e) => {
    ...
    ctx.strokeStyle = isEraser ? "#FFF9F0" : brushColor;
  ...
  const clear = () => {
    const cv = canvasRef.current, ctx = cv.getContext("2d");
    ctx.fillStyle = "#FFF9F0"; ctx.fillRect(0, 0, cv.width, cv.height); setIsEmpty(true);
  };
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", gap:4, marginBottom:8, flexWrap:"wrap" }}>
        ...
        <div style={{ width:1, height:18, background:"#E2D9CF", margin:"0 2px" }} />
        {BRUSH_SIZES.map(s => (
          <button key={s} onClick={() => { setBrushSize(s); setIsEraser(false); }}
            style={{ width:s*3+4, height:s*3+4, borderRadius:"50%", border:"none", cursor:"pointer", minHeight:44, flexShrink:0,
              background: brushSize===s && !isEraser ? "#3B2E22" : "#D3C9BC" }} />
        ))}
        <button onClick={() => setIsEraser(e => !e)}
          style={{ fontSize:15, background: isEraser ? "#F4B8C8" : "transparent",
            border:`1.5px solid ${isEraser?"#D4537E":"#D3C9BC"}`, borderRadius:8, padding:"1px 6px", cursor:"pointer", minHeight:44 }}>
          🧹
        </button>
      </div>
      <canvas ref={canvasRef} width={520} height={200}
        ...
        style={{ width:"100%", height:150, borderRadius:16, border:"1.5px solid #E2D9CF",
          background:"#FFF9F0", display:"block", cursor: isEraser ? "cell" : "crosshair", touchAction:"none" }} />
      <div style={{ display:"flex", gap:8, marginTop:8 }}>
        ...
        <button onClick={() => !isEmpty && onSave(canvasRef.current.toDataURL())} disabled={isEmpty}
          style={{ flex:2, padding:"8px 0", borderRadius:8, fontSize:11, fontWeight:700,
            border:"none", background: isEmpty ? "#D3C9BC" : color, color:"#fff",
...
function SceneBoubou({ count }) {
  ...
  return (
    <div style={{ background:"linear-gradient(135deg,#FFF0F6,#FFE4F0)", borderRadius:16, padding:"12px 14px 8px", border:"1.5px solid #F4B8C8", marginBottom:12 }}>
      <div style={{ fontSize:11, color:"#72243E", fontWeight:700, marginBottom:4 }}>
        🩹 {count} douceur{count>1?"s":""} à soigner ce soir
      </div>
      <div style={{ fontSize:11, color:"#C85A7A", marginBottom:8, fontStyle:"italic", lineHeight:1.5 }}>
      ...
      <svg width="100%" viewBox="0 0 300 90" style={{ display:"block", overflow:"visible" }}>
        ...
        <g transform="translate(106,8)">
          ...
          {/* Tête */}
          <circle cx="40" cy="34" r="22" fill="#F5F0EA"/>
          {/* Nez */}
        ...
        {[{x:30,y:20,s:0.7,d:0},{x:245,y:15,s:0.9,d:0.8},{x:270,y:45,s:0.6,d:1.4},{x:20,y:55,s:0.75,d:0.4}].map((h,i)=>(
          <motion.g key={i} animate={{ y:[h.y,h.y-18,h.y-30], opacity:[0,1,0], scale:[h.s,h.s*1.1,h.s*0.8] }}
            ...
            <path d={`M${h.x} ${h.y+4} Q${h.x} ${h.y} ${h.x+5} ${h.y+2} Q${h.x+10} ${h.y} ${h.x+10} ${h.y+4} Q${h.x+10} ${h.y+9} ${h.x+5} ${h.y+12} Q${h.x} ${h.y+9} ${h.x} ${h.y+4}Z`}
              fill={["#E8487A","#F08090","#D4537E","#FF90B0"][i]} opacity="0.85"/>
          </motion.g>
        ))}
        ...
        {[{x:60,y:12},{x:240,y:60},{x:12,y:38}].map((s,i)=>(
          <motion.g key={i}
            ...
            <text x={s.x} y={s.y} fontSize="10" textAnchor="middle" fill="#D4537E">✦</text>
          </motion.g>
        ))}
      </svg>
    </div>
...
function SceneVent({ count }) {
  ...
  return (
    <div style={{ background:"linear-gradient(135deg,#F0ECFF,#E4D8FF)", borderRadius:16, padding:"12px 14px 8px", border:"1.5px solid #C8B8F0", marginBottom:12 }}>
      <div style={{ fontSize:11, color:"#3C3489", fontWeight:700, marginBottom:4 }}>
...
function SceneVagues({ count }) {
  ...
  return (
    <div style={{ background:"linear-gradient(160deg,#1A4A7A,#1E6AAA,#28A0C8)", borderRadius:16, padding:"12px 14px 8px", border:"1.5px solid #1A6AAA", marginBottom:12 }}>
      <div style={{ fontSize:11, color:"#A8D8FF", fontWeight:700, marginBottom:4 }}>
      ...
      <svg width="100%" viewBox="0 0 300 90" style={{ display:"block", overflow:"visible" }}>
        <defs>
          ...
          <linearGradient id="skySeaGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%"   stopColor="#3AB0E8"/>
        ...
        <motion.g animate={{ x:[0,180,0], y:[0,-4,2,-3,0] }}
          ...
          {/* Voile 1 */}
          <path d="M18 28 L18 8 L30 18 Z" fill="#E8F4FF" stroke="#A8C8E8" strokeWidth="0.8" opacity="0.95"/>
          {/* Voile 2 */}
        ...
        <circle cx="274" cy="10" r="7"  fill="#3AB0E8" opacity="0.8"/>
      </svg>
    </div>
  );
}
...
function SceneLettre({ count }) {
  ...
  return (
    <div style={{ background:"linear-gradient(135deg,#C8D4F0,#D4DFFF,#E8EEFF)", borderRadius:16, padding:"12px 14px 8px", border:"1.5px solid #8098C8", marginBottom:12 }}>
      <div style={{ fontSize:11, color:"#0C1E50", fontWeight:700, marginBottom:4 }}>
...
function SceneBonheur({ count }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize:11, color:"#2A6B5A", fontWeight:700, marginBottom:8 }}>
      ...
      <svg width="100%" viewBox="0 0 300 120" style={{ display:"block", overflow:"visible" }}>
        <defs>
          ...
          <radialGradient id="pantGrad" cx="50%" cy="20%" r="80%">
            ...
            <stop offset="100%" stopColor="#5BAAE8"/>
          </radialGradient>
        ...
        <circle cx="40"  cy="30" r="18" fill="#A8E8D8" opacity="0.25"/>
        <circle cx="265" cy="25" r="14" fill="#F8C8DC" opacity="0.30"/>
        <circle cx="255" cy="85" r="10" fill="#FAC775" opacity="0.25"/>
        ...
        {Array.from({length:Math.min(count,5)},(_,i) => {
          const configs = [
            {x:20,  y:70, emoji:"⭐", col:"#FAC775"},
            {x:260, y:65, emoji:"💛", col:"#F5D030"},
            {x:15,  y:30, emoji:"🌟", col:"#FFE060"},
            {x:265, y:30, emoji:"✨", col:"#AFA9EC"},
...
function useMic({ onResult, onEnd }) {
  ...
  const start = useCallback(async () => {
    ...
    rec.onerror = (e) => {
      setActive(false);
      if (e.error === "not-allowed")    setMicError("Permission micro refusée");
      else if (e.error === "no-speech") setMicError("Rien entendu — réessaie !");
...
function ShelfPanel({ shelf, thoughts, onAdd, onRemove, speak, isNight, bookcaseRef }) {
  ...
  return (
    <>
      ...
      style={{
        background: isNight ? "rgba(20,16,50,0.92)" : shelf.bg,
      ...
      <div style={{ fontSize:13, fontWeight:700, color: isNight ? "#EEEDFE" : shelf.text, marginBottom:8 }}>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:8, minHeight:40, marginBottom:12 }}>
        ...
        {thoughts.length === 0 && (
          <span style={{ fontSize:13, color: isNight ? "#AFA9EC" : shelf.text+"99", fontStyle:"italic" }}>
          ...
          return (
            <button key={m}
              ...
              style={{ flex:1, padding:"9px 0", borderRadius:8, fontSize:13, fontWeight:700,
                ...
                color: disabled ? (isNight?"#555":"#ccc") : isModeActive ? (isNight?"#fff":shelf.dot) : (isNight?"#EEEDFE":shelf.text),
      ...
      {mode === "text" && (
        <div style={{ display:"flex", gap:8 }}>
          <input autoFocus value={val} onChange={e => setVal(e.target.value)}
            onKeyDown={e => e.key==="Enter" && submit()} placeholder={shelf.placeholder}
            style={{ flex:1, borderRadius:8, border:`1.5px solid ${isNight?shelf.dot+"66":shelf.border}`,
              ...
              padding:"9px 12px", fontSize:13, color: isNight ? "#EEEDFE" : "#3B2E22",
          ...
          <button ref={submitBtnRef} onClick={submit}
            style={{ background: shelf.dot, color:"#fff", border:"none", borderRadius:8,
              padding:"10px 12px", fontSize:13, fontWeight:700, cursor:"pointer", minHeight:44,
      ...
      {mode === "voice" && (
        <div>
          ...
          {micError && (
            <motion.div initial={{ opacity:0, y:-4 }} animate={{ opacity:1, y:0 }}
              style={{
                ...
                fontSize:13, color:"#712B13", fontWeight:600,
          ...
          <div style={{ minHeight:44, background:"#fff", borderRadius:8,
            ...
            fontFamily:FF.body, lineHeight:1.5,
          ...
          <div style={{ display:"flex", gap:8 }}>
            <motion.button
              ...
              style={{ flex:1, padding:"11px 0", borderRadius:16, border:"none", cursor:"pointer", minHeight:44,
                background: active ? "#D85A30" : shelf.dot,
                color:"#fff", fontSize:13, fontWeight:700, fontFamily:FF.body,
            ...
            {val && (
              <button ref={submitBtnRef} onClick={submit}
                style={{ flex:2, padding:"11px 0", borderRadius:16, border:"none",
                  background:shelf.dot, color:"#fff", fontSize:13, fontWeight:700,
...
function BookcaseSVG({ isOpen, thoughts, onShelfClick, activeShelf }) {
  ...
  const SB = [
    [{w:11,h:32,c:"#F4B8C8"},{w:13,h:40,c:"#FAC775"},{w:9,h:28,c:"#AFA9EC"},{w:12,h:36,c:"#9FE1CB"},{w:10,h:42,c:"#F0997B"},{w:14,h:30,c:"#D4537E"},
     {w:11,h:38,c:"#5DCAA5"},{w:9,h:34,c:"#7F77DD"},{w:13,h:26,c:"#85B7EB"},{w:10,h:44,c:"#EF9F27"},{w:12,h:32,c:"#ED93B1"},{w:8,h:38,c:"#639922"}],
    [{w:12,h:38,c:"#85B7EB"},{w:9,h:30,c:"#EF9F27"},{w:14,h:44,c:"#ED93B1"},{w:10,h:34,c:"#639922"},{w:11,h:40,c:"#378ADD"},{w:13,h:28,c:"#FAC775"},
     {w:8,h:36,c:"#D85A30"},{w:12,h:42,c:"#AFA9EC"},{w:10,h:32,c:"#F4B8C8"},{w:14,h:38,c:"#9FE1CB"},{w:9,h:26,c:"#7F77DD"},{w:11,h:34,c:"#F0997B"}],
    [{w:9,h:34,c:"#9FE1CB"},{w:12,h:40,c:"#F4B8C8"},{w:11,h:30,c:"#7F77DD"},{w:10,h:38,c:"#EF9F27"},{w:13,h:44,c:"#5DCAA5"},{w:8,h:28,c:"#D4537E"},
     {w:12,h:36,c:"#85B7EB"},{w:10,h:42,c:"#F0997B"},{w:14,h:32,c:"#FAC775"},{w:9,h:40,c:"#AFA9EC"},{w:11,h:28,c:"#ED93B1"},{w:13,h:36,c:"#378ADD"}],
    [{w:11,h:36,c:"#FAC775"},{w:9,h:30,c:"#AFA9EC"},{w:13,h:42,c:"#ED93B1"},{w:10,h:34,c:"#378ADD"},{w:12,h:44,c:"#9FE1CB"},{w:8,h:28,c:"#EF9F27"},
     {w:14,h:38,c:"#D85A30"},{w:11,h:32,c:"#639922"},{w:9,h:40,c:"#F4B8C8"},{w:13,h:36,c:"#7F77DD"},{w:10,h:26,c:"#85B7EB"},{w:12,h:34,c:"#F0997B"}],
    [{w:10,h:32,c:"#F4B8C8"},{w:13,h:40,c:"#7F77DD"},{w:9,h:28,c:"#85B7EB"},{w:12,h:36,c:"#FAC775"},{w:11,h:44,c:"#5DCAA5"},{w:8,h:30,c:"#D4537E"},
     {w:14,h:38,c:"#EF9F27"},{w:10,h:34,c:"#AFA9EC"},{w:13,h:42,c:"#378ADD"},{w:9,h:28,c:"#F0997B"},{w:11,h:36,c:"#ED93B1"},{w:12,h:30,c:"#639922"}],
  ];

  /* Livres dansants du bas — délais variés pour effet aléatoire */
  const BB = [
    {x:62, w:13,h:26,c:"#D4537E",d:0.0},{x:77, w:10,h:20,c:"#378ADD",d:0.35},
    {x:89, w:15,h:28,c:"#EF9F27",d:0.7},{x:106,w:10,h:22,c:"#1D9E75",d:1.1},
    {x:118,w:12,h:24,c:"#7F77DD",d:0.2},{x:132,w:9, h:18,c:"#F4B8C8",d:0.85},
    {x:312,w:14,h:25,c:"#D85A30",d:0.5},{x:328,w:11,h:30,c:"#5DCAA5",d:0.1},
    {x:341,w:9, h:20,c:"#FAC775",d:0.9},{x:352,w:13,h:26,c:"#AFA9EC",d:0.6},
    {x:427,w:15,h:27,c:"#EF9F27",d:0.3},{x:444,w:10,h:22,c:"#ED93B1",d:1.0},
    {x:456,w:13,h:25,c:"#639922",d:0.65},{x:471,w:11,h:20,c:"#378ADD",d:0.25},
  ];
  ...
  return (
    <svg width="100%" viewBox="0 0 560 520"
      ...
      {/* ═══════════════════════════════════════════════════
          ...
          4  Montant central  ← dessiné AVANT livres ET étiquettes
          5  Étagères + livres
          6  Charnières
          7  Socle
          8  Livres dansants bas
          9  Étiquettes cliquables  ← last → toujours devant
      ...
      {isOpen && SHELVES.map((shelf, i)=>{
        ...
        const POSTIT_COLORS = {
          boubou:"#FFE0EC", cloud:"#D6EAFF", trash:"#C8EED8",
          lettre:"#E4DCFF", bonheur:"#FFF0B8",
        };
        const pcol = POSTIT_COLORS[shelf.id] || "#FFF8E6";
        const pStartX = 305;
        const maxVisible = 6;

        return (
          <motion.g key={shelf.id} onClick={()=>onShelfClick(shelf.id)} style={{cursor:"pointer", minHeight:44}}
            ...
            {(() => {
              try {
                ...
                return (
                  <g>
                    <circle cx="470" cy={midY-14} r="10" fill="#EF9F27"
...
function SleepScreen({ bonheur, onReopen, onClearNight, profile = {} }) {
  ...
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:2 }}
      style={{ minHeight:"100dvh", background:`linear-gradient(180deg,${T.nuit.deep} 0%,${T.nuit.purple} 40%,#0E0830 100%)`,
      ...
      {Array.from({length:50},(_,i) => (
        <motion.div key={i} style={{ position:"absolute", borderRadius:"50%",
          background: i%5===0 ? "#FAC775" : i%7===0 ? "#ED93B1" : "#AFA9EC",
      ...
      <motion.div style={{ textAlign:"center", zIndex:1, maxWidth:420 }}
        ...
        <svg width="110" height="110" viewBox="0 0 110 110" style={{ margin:"0 auto 20px", display:"block" }}>
          ...
          {[{x:82,y:25,s:3},{x:22,y:30,s:2.5},{x:88,y:65,s:2},{x:18,y:72,s:3.5}].map((st,i) => (
            <motion.circle key={i} cx={st.x} cy={st.y} r={st.s} fill="#FAC775" opacity="0.8"
        ...
        {profile?.name && (
          <motion.div initial={{ opacity:0, scale:0.8 }} animate={{ opacity:1, scale:1 }}
            ...
            style={{ fontSize:19, fontWeight:800, color:"#FAC775", marginBottom:8,
              textShadow:"0 0 20px rgba(250,199,117,0.5)" }}>
            {profile?.avatar} Bonne nuit, {profile?.name} !
          </motion.div>
        )}
        <h1 style={{ fontSize:24, fontWeight:800, color:"#EEEDFE", marginBottom:8, lineHeight:1.3,
          textShadow:"0 0 30px rgba(174,169,236,0.4)" }}>
          La bibliothèque se ferme doucement…
        </h1>
        <p style={{ fontSize:13, color:"#AFA9EC", lineHeight:1.8, marginBottom:24 }}>
        ...
        {bonheur.length > 0 && (
          <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:1 }}
            style={{ background:"rgba(21,16,58,0.9)", borderRadius:16, padding:"16px 20px",
              ...
              boxShadow:"0 4px 24px rgba(83,74,183,0.2)" }}>
            <div style={{ fontSize:11, color:"#AFA9EC", marginBottom:8, textTransform:"uppercase", letterSpacing:"0.06em" }}>
              ⭐ Je garde dans mon cœur
            </div>
            {bonheur.map((t,i) => (
              <motion.div key={i} initial={{ opacity:0, x:-8 }} animate={{ opacity:1, x:0 }}
                transition={{ delay:1.2+i*0.18 }}
                style={{ fontSize:13, color:"#FAC775", padding:"11px 0", lineHeight:1.5,
            ...
            <button onClick={reading ? stop : readBonheur}
              style={{ marginTop:12, width:"100%", padding:"10px 0", borderRadius:16,
                border:"1px solid #534AB7", background: reading ? "#534AB7" : "transparent",
                color: reading ? "#fff" : "#AFA9EC", fontSize:13, fontWeight:600,
                cursor:"pointer", minHeight:44, fontFamily:FF.body,
                transition:"all 0.2s" }}>
        ...
        <p style={{ fontSize:13, color:"#7F77DD", marginBottom:32, lineHeight:1.7 }}>
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"center", flexWrap:"wrap" }}>
          <button onClick={onReopen}
            style={{ background:"transparent", border:"1.5px solid #534AB7", borderRadius:24,
              padding:"11px 22px", fontSize:13, color:"#AFA9EC", cursor:"pointer", minHeight:44,
              fontFamily:FF.body, transition:"all 0.2s" }}
            onMouseEnter={e => { e.target.style.background="#534AB7"; e.target.style.color="#fff"; }}
            onMouseLeave={e => { e.target.style.background="transparent"; e.target.style.color="#AFA9EC"; }}>
            Rouvrir la bibliothèque
          </button>
          <button onClick={onClearNight}
            style={{ background:"transparent", border:"1.5px solid #3C3489", borderRadius:24,
              padding:"11px 22px", fontSize:13, color:"#7F77DD", cursor:"pointer", minHeight:44,
              fontFamily:FF.body, transition:"all 0.2s" }}
            onMouseEnter={e => { e.target.style.background="#3C3489"; e.target.style.color="#fff"; }}
            onMouseLeave={e => { e.target.style.background="transparent"; e.target.style.color="#7F77DD"; }}>
            Nouvelle nuit 🌅
          </button>
        </div>
      </motion.div>
...
function Doudou({ size = 80, animate: doAnim = true, pose = "sit", style: extraStyle = {}, reduced = false }) {
  ...
  return (
    <motion.div
      style={{ display:"inline-block", lineHeight:0, ...extraStyle }}
...
function VoiceLoading() {
  return (
    <motion.div
      ...
      <div>
        <div style={{ fontSize:13, fontWeight:700, color:"#EEEDFE" }}>Le doudou prépare sa voix…</div>
        <div style={{ display:"flex", gap:4, marginTop:4 }}>
          {[0,1,2,3,4].map(i => (
            <motion.div key={i}
              animate={{ height:[3,12,3], background:["#AFA9EC","#EEEDFE","#AFA9EC"] }}
...
function ProfileSetup({ onDone }) {
  ...
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
      ...
      <motion.div animate={{ scale:[1,1.08,1] }} transition={{ duration:8, repeat:Infinity, delay:2 }}
      ...
      <motion.h2 initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.3 }}
        style={{ fontFamily:FF.title, fontSize:24,
          color:"#085041", marginBottom:8, textAlign:"center" }}>
      ...
      <input autoFocus value={name}
        ...
        style={{ width:"100%", maxWidth:340, padding:"16px 20px", borderRadius:24,
          ...
          fontSize:19, fontFamily:FF.body, color:"#085041",
          outline:"none", textAlign:"center", marginBottom:20,
          boxSizing:"border-box", boxShadow:"0 4px 20px #1D9E7522" }}
      />
...
function WelcomeScreen({ profile, onNext }) {
  ...
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
      ...
      {/* Blobs */}
      {[{t:"-10%",l:"-10%",w:220,d:0},{b:"-8%",r:"-8%",w:180,d:2},{t:"40%",l:"80%",w:120,d:1}].map((b,i)=>(
        <motion.div key={i} animate={{ scale:[1,1.1,1] }} transition={{ duration:6+i*2, repeat:Infinity, delay:b.d }}
          style={{ position:"absolute", top:b.t, left:b.l, bottom:b.b, right:b.r,
            width:b.w, height:b.w, borderRadius:"50%",
            background:"#1D9E7525", filter:"blur(36px)", pointerEvents:"none" }}/>
      ))}
      ...
      <motion.button whileTap={{ scale:0.95 }} whileHover={{ scale:1.04 }}
        ...
        style={{ padding:"17px 52px", border:"none", borderRadius:24,
          ...
          boxShadow:"0 10px 32px #1D9E7555" }}>
        Allons-y ! →
      </motion.button>
    </motion.div>
  );
  ...
  const slides = [
    {
      ...
      dot:"#1D9E75", bg:"linear-gradient(145deg,#E4FFF8,#C4F0E4)",
    },
    {
      emoji:"💭",
      ...
      dot:"#7F77DD", bg:"linear-gradient(145deg,#F4EEFF,#E4D4FF)",
    },
    {
      doudou:true,
      ...
      dot:"#EF9F27", bg:"linear-gradient(145deg,#FFFCE0,#FFF0A0)",
    },
  ];
        ...
        <motion.div key={step}
          ...
          {cur.doudou ? (
            ...
            <motion.div
              ...
              transition={{ duration:3.5, repeat:Infinity, ease:"easeInOut" }}
              style={{ fontSize:90, marginBottom:16, lineHeight:1,
          ...
          <p style={{ fontSize:15, color:"#2A2010", lineHeight:1.85,
...
function CompanionScreen({ profile, onDone }) {
  ...
  useEffect(() => {
    speak("Choisis ton compagnon du soir. Celui qui veillera sur toi chaque nuit.", { rate:0.80, pitch:0.92 });
    return () => stop();
  }, []);
  ...
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
      style={{ position:"fixed", inset:0, zIndex:100,
        background:"linear-gradient(160deg,#FFF8E0,#FFF0A8,#FFE060)",
        display:"flex", flexDirection:"column",
        alignItems:"center", justifyContent:"center",
        padding:"24px 20px", fontFamily:FF.body, overflow:"hidden" }}>
      ...
      <motion.div animate={{ scale:[1,1.1,1] }} transition={{ duration:6, repeat:Infinity }}
        style={{ position:"absolute", top:"-10%", left:"-10%", width:220, height:220,
          borderRadius:"50%", background:"#EF9F2722", filter:"blur(36px)", pointerEvents:"none" }}/>
      <motion.div animate={{ scale:[1,1.08,1] }} transition={{ duration:8, repeat:Infinity, delay:2 }}
        style={{ position:"absolute", bottom:"-8%", right:"-8%", width:180, height:180,
          borderRadius:"50%", background:"#EF9F2730", filter:"blur(30px)", pointerEvents:"none" }}/>

      {/* Avatar sélectionné animé */}
      <motion.div animate={{ y:[0,-12,0], scale:[1,1.08,1] }}
        transition={{ duration:2.5, repeat:Infinity, ease:"easeInOut" }}
        style={{ fontSize:80, marginBottom:8, lineHeight:1,
          filter:"drop-shadow(0 8px 20px rgba(239,159,39,0.4))" }}>
        {avatar}
      </motion.div>
      <div style={{ fontFamily:FF.title, fontSize:19,
        color:"#633806", marginBottom:20 }}>
      ...
      <p style={{ fontSize:13, color:"#7A5020", marginBottom:20, textAlign:"center" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:8,
        ...
        borderRadius:24, padding:14, marginBottom:24, maxWidth:380, width:"100%",
        boxShadow:"0 6px 24px rgba(239,159,39,0.2)" }}>
        {AVATARS.map(a => (
          <motion.button key={a} onClick={() => { setAvatar(a); speak(AVATAR_NAMES[a]||a, {rate:0.9}); }}
            whileTap={{ scale:0.88 }} whileHover={{ scale:1.18 }}
            style={{ fontSize:32, background: avatar===a ? "rgba(239,159,39,0.25)" : "transparent",
              border: avatar===a ? "2.5px solid #EF9F27" : "2px solid transparent",
              borderRadius:16, padding:"8px 4px", cursor:"pointer", minHeight:44,
              transition:"all 0.15s",
              boxShadow: avatar===a ? "0 3px 10px rgba(239,159,39,0.3)" : "none" }}>
            {a}
          </motion.button>
        ))}
      </div>
      ...
      <motion.button whileTap={{ scale:0.95 }} whileHover={{ scale:1.04 }}
        onClick={confirm}
        style={{ padding:"17px 52px", border:"none", borderRadius:24,
          background:"linear-gradient(135deg,#EF9F27,#D88A18)",
          color:"#fff", fontFamily:FF.title,
          fontSize:19, cursor:"pointer", minHeight:44,
          boxShadow:"0 10px 30px rgba(239,159,39,0.5)" }}>
        C'est lui ! ✨
      </motion.button>
    </motion.div>
  );
...
function Onboarding({ onDone, childName }) {
  const [step, setStep]       = useState(0);
      ...
      <AnimatePresence mode="wait">
        <motion.div key={step}
          ...
          <motion.div
            ...
            style={{ fontSize:100, lineHeight:1, marginBottom:24,
          ...
          <motion.h2
            ...
            style={{ fontFamily:FF.title, fontSize:24,
              color:"#1A2820", marginBottom:16, lineHeight:1.25,
          ...
          <p style={{ fontSize:15, lineHeight:1.85, marginBottom:32,
          ...
          <motion.button whileTap={{ scale:0.95 }} whileHover={{ scale:1.05, y:-3 }}
            ...
            style={{ width:"100%", padding:"18px 0",
              ...
              fontSize:19, fontWeight:800, cursor:"pointer", minHeight:44,
...
const PIN_INPUT_STYLE = {
  ...
  background:"#1E1A3A", color:"#EEEDFE", fontSize:15, outline:"none",
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:"16px", border:"1px solid #E2D9CF" }}>
        <div style={{ fontSize:13, fontWeight:700, color:T.neutral.brown, marginBottom:8 }}>🎙️ Voix du doudou</div>

        {/* Statut */}
        <div style={{
          fontSize:11, padding:"8px 12px", borderRadius:8, marginBottom:12,
          background: activeVoice==="ElevenLabs" ? "#E8F5EB" : activeVoice==="OpenAI" ? "#EEF4FF" : "#FFF8E6",
          color: activeVoice==="ElevenLabs" ? "#085041" : activeVoice==="OpenAI" ? "#3C3489" : "#633806",
          fontWeight:600,
        }}>
        ...
        {/* ElevenLabs */}
        <div style={{ marginBottom:16, padding:"12px", background:"#F8F4EF", borderRadius:16 }}>
          <div style={{ fontSize:11, fontWeight:800, color:"#3C3489", marginBottom:8 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:4, marginBottom:8 }}>
            {VOICES.map(v => (
              <button key={v.id} onClick={() => setVoiceId(v.id)}
                ...
                <span style={{ fontSize:11, fontWeight:700, color: voiceId===v.id ? "#3C3489" : "#3B2E22" }}>
                  {v.name} — <span style={{ fontWeight:400 }}>{v.desc}</span>
                </span>
                {voiceId===v.id && <span style={{ color:"#7F77DD" }}>✓</span>}
              </button>
            ))}
          </div>
          <div style={{ display:"flex", gap:8 }}>
          ...
          <div style={{ fontSize:11, color:"#9B7A55", marginTop:4 }}>elevenlabs.io → My Account → API Keys</div>
        </div>
        ...
        {keyMsg && <div style={{ fontSize:11, color:"#1D9E75", marginBottom:8, fontWeight:700 }}>{keyMsg}</div>}
        <button onClick={saveApiKey}
          style={{ width:"100%", padding:"11px", borderRadius:16, border:"none",
            background: (apiKey.trim()||openaiKey.trim()) ? "#7F77DD" : "#D3C9BC",
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:"16px", border:"1px solid #E2D9CF" }}>
        <div style={{ fontSize:13, fontWeight:700, color:T.neutral.brown, marginBottom:12 }}>🔑 Code PIN</div>
        {!changingPin ? (
          <button onClick={() => setChangingPin(true)}
            style={{ width:"100%", padding:"11px", borderRadius:16, fontSize:13,
              background:"#F0EAE2", border:"1px solid #E2D9CF", color:T.neutral.brown,
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            ...
            {pinMsg && <div style={{ fontSize:13, color: pinMsg.startsWith("✓") ? "#1D9E75" : "#D85A30" }}>{pinMsg}</div>}
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={savePin}
                style={{ flex:2, padding:"9px", borderRadius:8, border:"none",
                  background:"#3B2E22", color:"#fff", fontSize:13, fontWeight:700,
                  cursor:"pointer", minHeight:44, fontFamily:FF.body }}>
                Enregistrer
              </button>
            </div>
      ...
      {/* Âge / taille police */}
      <div style={{ background:"#fff", borderRadius:16, padding:"16px", border:"1px solid #E2D9CF" }}>
        <div style={{ fontSize:13, fontWeight:700, color:T.neutral.brown, marginBottom:12 }}>👧 Profil d'âge</div>
          ...
          return (
            <button key={mode}
              ...
              style={{ width:"100%", padding:"10px 12px", borderRadius:16, marginBottom:8,
                background: active ? "#F0EEFF" : "#F8F4EF",
                border: active ? "1.5px solid #7F77DD" : "1px solid #E2D9CF",
                cursor:"pointer", minHeight:44, fontFamily:FF.body,
                display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div style={{ textAlign:"left" }}>
                <div style={{ fontSize:13, fontWeight:700, color: active ? "#3C3489" : "#3B2E22" }}>{label}</div>
                <div style={{ fontSize:11, color:T.neutral.brownMid }}>{desc}</div>
              </div>
              {active && <span style={{ color:"#7F77DD" }}>✓</span>}
            </button>
          );
        })}
      </div>
      ...
      {/* Export */}
      <div style={{ background:"#fff", borderRadius:16, padding:"16px", border:"1px solid #E2D9CF" }}>
        <div style={{ fontSize:13, fontWeight:700, color:T.neutral.brown, marginBottom:12 }}>📄 Export</div>
        <button onClick={() => {
          const profile = loadProfile();
          const raw = JSON.parse(localStorage.getItem("bibl_pensees_v1")||"{}");
          const hist = loadHistory();
          ...
          style={{ width:"100%", padding:"11px", borderRadius:16, fontSize:13,
            background:"#F0EEFF", border:"1px solid #C8BFEE", color:"#3C3489",
            cursor:"pointer", minHeight:44, fontFamily:FF.body, textAlign:"left" }}>
          📥 Télécharger les pensées (.txt)
        </button>
        <div style={{ fontSize:11, color:T.neutral.brownMid, marginTop:8, lineHeight:1.5 }}>
          Fichier texte à partager avec un professionnel de santé
        </div>
      </div>
      <div style={{ background:"#fff", borderRadius:16, padding:"16px", border:"1px solid #E2D9CF" }}>
        <div style={{ fontSize:13, fontWeight:700, color:T.neutral.brown, marginBottom:12 }}>🗑️ Données</div>
        {!confirmClear ? (
          ...
          <div>
            <div style={{ fontSize:13, color:"#712B13", marginBottom:8, lineHeight:1.5 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={onClearNight}
                style={{ flex:2, padding:"9px", borderRadius:8, border:"none",
                  background:"#D85A30", color:"#fff", fontSize:13, fontWeight:700,
                  cursor:"pointer", minHeight:44, fontFamily:FF.body }}>
                Oui, effacer
              </button>
            </div>
      ...
      <div style={{ background:"#EEF4FB", borderRadius:16, padding:"14px 16px", border:"1px solid #B5D4F4" }}>
        <div style={{ fontSize:11, color:"#0C447C", lineHeight:1.7 }}>
...
function ParentSpace({ thoughts, onClose, onClearNight }) {
  ...
  const [pinSet]                = useState(!!localStorage.getItem(PIN_KEY));
  ...
  if (phase === "pin") return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
      style={{ position:"fixed", inset:0, zIndex:200, background:"rgba(15,12,42,0.93)",
        display:"flex", alignItems:"center", justifyContent:"center",
      ...
      <motion.div initial={{ y:30, opacity:0 }} animate={{ y:0, opacity:1 }}
        ...
        <h2 style={{ fontSize:19, fontWeight:800, color:"#EEEDFE", marginBottom:8 }}>Espace parent</h2>
        <p style={{ fontSize:13, color:"#AFA9EC", marginBottom:24, lineHeight:1.6 }}>
        ...
        {!pinSet ? (
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            ...
            {pinError && <div style={{ fontSize:13, color:"#F0997B" }}>{pinError}</div>}
            <button onClick={createPin}
              style={{ width:"100%", padding:"13px 0", borderRadius:16, border:"none",
                background:"#534AB7", color:"#fff", fontSize:15, fontWeight:700,
                cursor:"pointer", minHeight:44, fontFamily:FF.body }}>
              Créer mon code 🔒
            </button>
          </div>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:12, marginBottom:16 }}>
              {[0,1,2,3].map(i => (
                <div key={i} style={{ width:14, height:14, borderRadius:"50%", transition:"background 0.15s",
                  background: input.length>i ? "#AFA9EC" : "#534AB766", border:"1.5px solid #534AB7" }} />
              ))}
            </div>
              ...
              {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((k,i) => (
                <button key={i}
                  ...
                  style={{ padding:"13px 0", borderRadius:16, fontSize:19, fontWeight:700,
                    background: k==="" ? "transparent" : error ? "#4A1A1A" : "#1E1A3A",
                    border: k==="" ? "none" : `1px solid ${error?"#D85A30":"#534AB7"}`,
                    color: error ? "#F0997B" : "#EEEDFE", cursor: k==="" ? "default" : "pointer",
                    fontFamily:FF.body }}>
                  {k}
                </button>
              ))}
            </div>
            {error && <div style={{ fontSize:13, color:"#F0997B", marginBottom:8 }}>Code incorrect</div>}
          </>
        )}
        <button onClick={onClose}
          style={{ marginTop:16, background:"none", border:"none", fontSize:13,
            color:"#534AB7", cursor:"pointer", minHeight:44, fontFamily:FF.body }}>
          Annuler
        </button>
      </motion.div>
    </motion.div>
  ...
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"20px 20px 12px" }}>
        <div>
          <h1 style={{ fontSize:19, fontWeight:800, color:T.neutral.brown, margin:0 }}>🐻 Espace parent</h1>
          <p style={{ fontSize:13, color:T.neutral.brownMid, margin:0 }}>{total} pensée{total>1?"s":""} ce soir</p>
        </div>
        <button onClick={onClose}
          style={{ background:"#F0EAE2", border:"none", borderRadius:"50%",
      ...
      <div style={{ display:"flex", gap:8, padding:"0 20px 16px" }}>
        {[["resume","📊 Résumé"],["history","📋 Historique"],["stats","📈 7 jours"],["help","❓ Aide"],["settings","⚙️ Réglages"]].map(([id,lbl]) => (
          <button key={id} onClick={() => setTab(id)}
            style={{ flex:1, padding:"8px 0", borderRadius:16, fontSize:13, fontWeight:700,
              background: tab===id ? "#3B2E22" : "#F0EAE2",
              color: tab===id ? "#FDF8F1" : "#7A6E65",
      ...
      <div style={{ padding:"0 20px 40px" }}>
        {tab === "resume" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ background:"#fff", borderRadius:16, padding:"16px", border:"1px solid #E2D9CF" }}>
              <div style={{ fontSize:13, fontWeight:700, color:T.neutral.brown, marginBottom:12 }}>
              ...
              {countByShelf.map(s => (
                <div key={s.id} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ height:6, borderRadius:3, marginTop:4, background:"#F0EAE2", overflow:"hidden" }}>
                  ...
                  <span style={{ fontSize:13, fontWeight:700, color:s.dot, width:16, textAlign:"right" }}>{s.count}</span>
                </div>
              ))}
            </div>
            {thoughts.bonheur.length > 0 && (
              <div style={{ background:"#FFF8E6", borderRadius:16, padding:"16px", border:"1px solid #FAC775" }}>
                <div style={{ fontSize:13, fontWeight:700, color:"#633806", marginBottom:8 }}>⭐ Pensées bonheur ce soir</div>
                {thoughts.bonheur.map((item,i) => (
                  <div key={i} style={{ fontSize:13, color:"#854F0B", padding:"11px 0", lineHeight:1.5,
              ...
              : allEntries.map((entry,i) => {
                  ...
                  return (
                    <div key={i} style={{ background:"#fff", borderRadius:16, padding:"12px 14px",
                      border:`1px solid ${s?.border||"#E2D9CF"}`, display:"flex", alignItems:"flex-start", gap:8 }}>
                      <div style={{ width:32, height:32, borderRadius:8, flexShrink:0,
                        background:s?.bg||"#F5EDE3", border:`1px solid ${s?.border||"#E2D9CF"}`,
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        {entry.drawing
                          ...
                          : <div style={{ fontSize:13, color:T.neutral.brown, lineHeight:1.5 }}>{entry.text}</div>
                        }
                        ...
                        {replies[entry.id || `${entry.shelf}-${i}`] && (
                          <div style={{
                            marginTop:8, background:"#FFF8E6",
                            borderRadius:8, padding:"11px 10px",
                            borderLeft:`3px solid #FAC775`,
                            fontSize:13, color:"#633806", lineHeight:1.5,
                          }}>
                          ...
                          !entry.drawing && (
                            <button onClick={() => { setReplyIdx(entry.id || `${entry.shelf}-${i}`); setReplyText(""); }}
                              style={{ marginTop:4, background:"none", border:"none",
                                fontSize:11, color:"#EF9F27", cursor:"pointer", minHeight:44,
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            {[
              ...
              <div key={i} style={{ background:"#fff", borderRadius:16, padding:"14px 16px",
                ...
                <div style={{ fontSize:13, fontWeight:800, color:T.neutral.brown, marginBottom:8 }}>
                  {item.emoji} {item.q}
                </div>
                <div style={{ fontSize:13, color:"#5A4A3A", lineHeight:1.7 }}>{item.a}</div>
              </div>
            ))}
            <div style={{ background:"#EEF4FB", borderRadius:16, padding:"14px 16px",
              border:"1px solid #B5D4F4", fontSize:11, color:"#0C447C", lineHeight:1.7 }}>
        ...
        {tab === "stats" && (() => {
          ...
          const shelfColors = { boubou:"#D4537E", cloud:"#378ADD", trash:"#1D9E75", lettre:"#7F77DD", bonheur:"#EF9F27" };
          const maxVal = Math.max(...last7.map(n => Object.values(n.thoughts||{}).flat().length), 1);
          return (
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <div style={{ background:"#fff", borderRadius:16, padding:"16px", border:"1px solid #E2D9CF" }}>
                <div style={{ fontSize:13, fontWeight:700, color:T.neutral.brown, marginBottom:16 }}>
                    ...
                    {last7.map((night, i) => {
                      ...
                      return (
                        <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
                          <div style={{ fontSize:11, fontWeight:700, color:"#5A3A20" }}>{total||""}</div>
                          ...
                          <div style={{ fontSize:11, color:"#9B7A55", textTransform:"capitalize" }}>{label}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div style={{ background:"#fff", borderRadius:16, padding:"14px 16px", border:"1px solid #E2D9CF" }}>
                <div style={{ fontSize:11, fontWeight:700, color:T.neutral.brown, marginBottom:8 }}>Légende</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                  {SHELVES.map(s => (
                    <div key={s.id} style={{ display:"flex", alignItems:"center", gap:4 }}>
                      <div style={{ width:10, height:10, borderRadius:2, background:shelfColors[s.id] }}/>
                      <span style={{ fontSize:11, color:"#5A3A20" }}>{s.emoji} {s.label.split(" ").slice(0,2).join(" ")}</span>
                    </div>
                  ))}
                </div>
              </div>
    ...
    <motion.div
      ...
      style={{
        ...
        background: shelfId==="lettre"  ? "#E8D8FF" :
                    shelfId==="bonheur" ? "#FFF0B8" :
                    shelfId==="boubou"  ? "#FFE0EC" :
                    shelfId==="cloud"   ? "#D6EAFF" : "#C8EED8",
...
const FLOATERS = [
  {type:"cloud", x:"4%",  top:"18%", s:1,    d:0},
  {type:"cloud", x:"82%", top:"12%", s:0.7,  d:1.4},
  {type:"cloud", x:"88%", top:"52%", s:0.85, d:0.7},
  {type:"cloud", x:"2%",  top:"60%", s:0.75, d:2.1},
  {type:"cloud", x:"75%", top:"78%", s:0.6,  d:1.8},
  {type:"cloud", x:"10%", top:"88%", s:0.9,  d:0.4},
  {type:"heart", x:"12%", top:"8%",  s:1,    d:0.3},
  {type:"heart", x:"78%", top:"30%", s:0.8,  d:1.1},
  {type:"heart", x:"6%",  top:"42%", s:0.7,  d:1.7},
  {type:"heart", x:"86%", top:"68%", s:0.9,  d:0.6},
  {type:"heart", x:"20%", top:"72%", s:0.65, d:2.3},
  {type:"star",  x:"22%", top:"5%",  s:1,    d:0.5},
  {type:"star",  x:"70%", top:"5%",  s:0.8,  d:1.3},
  {type:"star",  x:"92%", top:"38%", s:0.7,  d:0.9},
  {type:"star",  x:"3%",  top:"30%", s:0.9,  d:1.6},
  {type:"star",  x:"60%", top:"88%", s:0.75, d:2.0},
];

function Floater({ type, d }) {
  if (type === "cloud") return (
    <svg width="62" height="36" viewBox="0 0 62 36">
      <ellipse cx="22" cy="26" rx="18" ry="11" fill="#fff" opacity="0.75"/>
      <ellipse cx="36" cy="21" rx="16" ry="12" fill="#fff" opacity="0.82"/>
      <ellipse cx="50" cy="26" rx="13" ry="9"  fill="#fff" opacity="0.75"/>
      <ellipse cx="31" cy="16" rx="12" ry="10" fill="#fff" opacity="0.88"/>
      {/* cloud shadow line */}
      <ellipse cx="36" cy="36" rx="20" ry="3" fill="rgba(180,160,140,0.12)"/>
    </svg>
  );
  if (type === "heart") return (
    <motion.svg width="32" height="30" viewBox="0 0 32 30"
      animate={{ scale:[1,1.2,1] }} transition={{ duration:1.6+d*0.3, repeat:Infinity, ease:"easeInOut" }}>
      <path d="M16 27 Q4 18 4 10 Q4 3 10 3 Q13.5 3 16 7.5 Q18.5 3 22 3 Q28 3 28 10 Q28 18 16 27Z"
        fill="#85B7EB" opacity="0.88"/>
      {/* heart shine */}
      <ellipse cx="10" cy="8" rx="2.5" ry="2" fill="rgba(255,255,255,0.5)" transform="rotate(-30,10,8)"/>
    </motion.svg>
  );
  return (
    <motion.svg width="26" height="26" viewBox="0 0 26 26"
      animate={{ rotate:[0,18,-12,6,0], opacity:[0.55,1,0.55] }}
      transition={{ duration:2.2+d*0.4, repeat:Infinity }}>
      <path d="M13 2 L15.5 9.5 H23 L17 14 L19.5 21.5 L13 17 L6.5 21.5 L9 14 L3 9.5 H10.5 Z"
        fill="#FAC775" opacity="0.92"/>
      <circle cx="13" cy="13" r="2.5" fill="#fff" opacity="0.4"/>
    </motion.svg>
  );
}

/* ══════════════════════════════════════════════════════════
   ÉCRANS LÉGAUX — Politique de confidentialité & CGU
══════════════════════════════════════════════════════════ */
const LEGAL_CONTENT = {
  privacy: {
    title: "Politique de confidentialité",
    emoji: "🔒",
    lastUpdate: "Mise à jour : mai 2025",
    sections: [
      {
        title: "1. Qui sommes-nous ?",
        content: "La Bibliothèque des Pensées est une application de bien-être émotionnel destinée aux enfants de 4 à 12 ans, développée avec soin par Amotessa SARL-S, dont le siège social est situé à 1 rue Charles Gounod, Luxembourg. Pour toute question : contact@amotessa.lu"
      },
      {
        title: "2. Quelles données sont collectées ?",
        content: "Aucune donnée personnelle n'est collectée, transmise ou stockée sur un serveur externe. L'application fonctionne entièrement hors ligne. Les pensées, dessins et notes saisies par l'enfant sont enregistrés uniquement sur l'appareil de l'utilisateur (mémoire locale). Aucune information ne quitte jamais l'appareil."
      },
      {
        title: "3. Données des enfants — Protection renforcée",
        content: "Conformément au Règlement Général sur la Protection des Données (RGPD), à l'article 8 relatif aux mineurs, et à la loi américaine COPPA, cette application a été conçue pour ne traiter aucune donnée relative aux enfants. Aucun compte utilisateur, aucune adresse e-mail, aucun identifiant n'est requis. L'espace parent protégé par code PIN permet aux parents de superviser l'utilisation de l'application."
      },
      {
        title: "4. Publicité",
        content: "L'application ne contient aucune publicité, aucun traceur publicitaire, et aucun SDK d'analyse comportementale. Aucun profil publicitaire n'est constitué."
      },
      {
        title: "5. Cookies et traceurs",
        content: "L'application n'utilise pas de cookies. Le stockage local (localStorage) utilisé sur la version web est strictement limité aux données de l'application (pensées, réglages, PIN chiffré). Ces données ne sont jamais partagées."
      },
      {
        title: "6. Partage des données",
        content: "Nous ne vendons, ne partageons et ne transmettons aucune donnée à des tiers, qu'il s'agisse de partenaires commerciaux, de réseaux publicitaires ou d'organismes gouvernementaux, sauf obligation légale impérative."
      },
      {
        title: "7. Durée de conservation",
        content: "Les données restent sur l'appareil tant que l'utilisateur ne les efface pas. La suppression de l'application entraîne la suppression définitive de toutes les données stockées localement. Aucune sauvegarde n'est effectuée sur nos serveurs."
      },
      {
        title: "8. Droits des utilisateurs",
        content: "Conformément au RGPD, vous disposez d'un droit d'accès, de rectification, d'effacement et d'opposition. Étant donné qu'aucune donnée n'est stockée hors de l'appareil, l'exercice de ces droits s'effectue directement via la fonction « Effacer toutes les données » accessible dans l'espace parent de l'application."
      },
      {
        title: "9. Sécurité",
        content: "L'espace parent est protégé par un code PIN. Le contenu de l'application est accessible uniquement sur l'appareil physique. Aucune connexion réseau n'est utilisée pour le fonctionnement de l'application."
      },
      {
        title: "10. Modifications",
        content: "Toute modification de cette politique sera signalée dans l'application lors de la mise à jour. La date de dernière modification est indiquée en haut de ce document."
      },
      {
        title: "11. Contact & réclamations",
        content: "Pour toute question relative à la confidentialité : contact@amotessa.lu. En cas de réclamation non résolue, vous pouvez saisir l'autorité de contrôle compétente : CNPD (Luxembourg), CNIL (France), ou l'autorité nationale de votre pays de résidence."
      },
    ]
  },
  cgu: {
    ...
    sections: [
      {
        ...
        content: "Les présentes Conditions Générales d'Utilisation (CGU) régissent l'utilisation de l'application La Bibliothèque des Pensées. En utilisant l'application, l'utilisateur (le parent ou tuteur légal de l'enfant) accepte sans réserve les présentes conditions. L'application est destinée aux enfants de 4 à 12 ans, sous la supervision d'un adulte responsable."
      },
      {
        title: "2. Description du service",
        content: "La Bibliothèque des Pensées est un outil numérique de bien-être émotionnel permettant à l'enfant de déposer ses pensées du soir sous forme de textes, de dessins ou d'enregistrements vocaux, dans un environnement sécurisé, bienveillant et sans connexion réseau. L'application propose également des mélodies apaisantes et un guide de fabrication d'une bibliothèque physique."
      },
      {
        title: "3. Accès et responsabilité parentale",
        content: "L'application est destinée à être utilisée par des enfants sous la supervision directe ou indirecte de leurs parents ou tuteurs légaux. L'espace parent, accessible via un code PIN, permet aux adultes de consulter les pensées enregistrées, de modifier les réglages et d'effacer les données. Il est de la responsabilité des parents de configurer cet espace et de s'assurer d'un usage adapté à l'âge de l'enfant."
      },
      {
        title: "4. Utilisation appropriée",
        content: "L'application est conçue pour un usage bienveillant et positif. Il est interdit d'utiliser l'application pour enregistrer des contenus illicites, violents, discriminatoires ou portant atteinte à la dignité des personnes. En cas d'utilisation inappropriée, l'éditeur se réserve le droit de prendre toute mesure nécessaire."
      },
      {
        title: "5. Propriété intellectuelle",
        content: "L'ensemble des éléments de l'application (design, illustrations, textes, sons, code source, animations) est la propriété exclusive de Amotessa SARL-S. Toute reproduction, modification, distribution ou utilisation commerciale sans autorisation écrite préalable est strictement interdite. L'utilisation personnelle et non commerciale est autorisée."
      },
      {
        title: "6. Contenu généré par l'utilisateur",
        content: "Les pensées, dessins et enregistrements créés par l'enfant restent sa propriété (ou celle de ses parents) et ne sont jamais transmis à l'éditeur. L'éditeur n'a aucun accès à ce contenu, qui est stocké uniquement sur l'appareil de l'utilisateur."
      },
      {
        title: "7. Disponibilité du service",
        content: "L'application fonctionne entièrement hors ligne. Aucune garantie de disponibilité d'un service en ligne n'est applicable. L'éditeur s'engage à corriger les bugs identifiés dans des délais raisonnables via des mises à jour des stores (App Store, Google Play)."
      },
      {
        title: "8. Limitation de responsabilité",
        content: "L'application est un outil de soutien au bien-être émotionnel et ne constitue en aucun cas un dispositif médical, thérapeutique ou de soins. Elle ne remplace pas l'accompagnement d'un professionnel de santé (pédopsychologue, orthophoniste, médecin). En cas de difficultés émotionnelles importantes chez l'enfant, nous encourageons vivement les parents à consulter un professionnel qualifié."
      },
      {
        title: "9. Mises à jour",
        content: "L'éditeur se réserve le droit de modifier les fonctionnalités de l'application via des mises à jour. Les CGU peuvent être modifiées à tout moment ; la version en vigueur est celle accessible dans l'application. La poursuite de l'utilisation après modification vaut acceptation des nouvelles conditions."
      },
      {
        title: "10. Droit applicable et juridiction",
        content: "Les présentes CGU sont soumises au droit luxembourgeois. En cas de litige, et après tentative de résolution amiable, les tribunaux du Grand-Duché de Luxembourg seront compétents. Pour les utilisateurs résidant dans un autre État membre de l'Union Européenne, les dispositions impératives du droit local s'appliquent."
      },
      {
        title: "11. Contact",
        content: "Pour toute question relative aux présentes CGU : contact@amotessa.lu — Amotessa SARL-S, 1 rue Charles Gounod, L-1118 Luxembourg, Grand-Duché de Luxembourg."
      },
    ]
  }
};
    ...
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      ...
      <motion.div
        ...
        style={{
          ...
          maxHeight:"88vh",
          display:"flex", flexDirection:"column",
          boxShadow:"0 -8px 40px rgba(59,46,34,0.18)",
        }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <h2 style={{ fontSize:19, fontWeight:800, color:T.neutral.brown, margin:0 }}>{content.title}</h2>
            <div style={{ fontSize:11, color:T.neutral.brownMid, marginTop:4 }}>{content.lastUpdate}</div>
          </div>
          <button onClick={onClose} style={{
            background:"#F0EAE2", border:"none", borderRadius:"50%",
        ...
        {type === "privacy" && (
          <div style={{
            margin:"14px 20px 0",
            background:"#E8F5EB", borderRadius:16, padding:"12px 16px",
            border:"1px solid #9FE1CB", flexShrink:0,
          }}>
            <div style={{ fontSize:13, color:"#085041", fontWeight:700, marginBottom:4 }}>
        ...
        {type === "cgu" && (
          <div style={{
            margin:"14px 20px 0",
            background:"#F0EEFF", borderRadius:16, padding:"12px 16px",
            border:"1px solid #C8BFEE", flexShrink:0,
          }}>
            <div style={{ fontSize:13, color:"#3C3489", fontWeight:700, marginBottom:4 }}>
              ⚠️ Point important
            </div>
        ...
        <div ref={scrollRef} style={{
          ...
          {content.sections.map((sec, i) => (
            <div key={i} style={{ marginBottom:20 }}>
              <div style={{
                fontSize:13, fontWeight:800, color:"#5A3A20",
                marginBottom:8, display:"flex", alignItems:"center", gap:8,
              }}>
                <span style={{
                  background:"#F0EAE2", borderRadius:8,
                  padding:"11px 8px", fontSize:11, color:"#9B7A55",
              ...
              <div style={{
                fontSize:13.5, color:"#5A4A3A", lineHeight:1.75,
                background:"#FDFAF6", borderRadius:8, padding:"10px 14px",
          ...
          {/* Legal footer */}
          <div style={{
            background:"#F5EDE3", borderRadius:16, padding:"14px 16px",
            border:"1px solid #E2D4C4", marginTop:8,
          }}>
            <div style={{ fontSize:11, color:T.neutral.brownMid, lineHeight:1.7 }}>
              <strong>Version :</strong> 1.0 — Juin 2025<br/>
              <strong>Éditeur :</strong> Amotessa SARL-S<br/>
              <strong>Contact :</strong> contact@amotessa.lu<br/>
...
export default function BibliothequeDesPensees() {
  ...
  const total = Object.values(thoughts).flat().length;
  const filledShelves = Object.values(thoughts).filter(a => a.length > 0).length;
  ...
  const handleAdd = (shelfId, payload) => {
  ...
  /* ══════════════════════════════════════════════════════
     ...
     2. WelcomeScreen    → bienvenue personnalisée
     3. PresentationScreen → à quoi ça sert
     4. CompanionScreen  → choix du compagnon
     5. Onboarding       → présentation des 6 étagères
     6. Bibliothèque     → app principale
     ══════════════════════════════════════════════════════ */
    ...
    <>
      ...
      {!STORAGE_OK && (
        <div style={{ position:"fixed", top:0, left:0, right:0, zIndex:400,
          ...
          fontSize:13, fontWeight:700, textAlign:"center", fontFamily:FF.body }}>
      ...
      <div style={{ minHeight:"100dvh",
        ...
        <div style={{ textAlign:"center", padding:"20px 16px 4px", position:"relative", zIndex:1 }}>
          <h1 style={{
            ...
            color: isNight ? "#A8E8D8" : "#2A6B5A", marginBottom:4,
            letterSpacing:"0.02em",
            textShadow: isNight
        ...
        <AnimatePresence>
          {celebrate && (
            <motion.div initial={{ opacity:0, y:-16, scale:0.9 }} animate={{ opacity:1, y:0, scale:1 }} exit={{ opacity:0 }}
              ...
              <div style={{ background:"#FFF8E6", border:"2px solid #FAC775", borderRadius:16,
                padding:"14px 18px", textAlign:"center" }}>
                <div style={{ fontSize:24, marginBottom:4 }}>🌟✨🌟</div>
                <div style={{ fontSize:15, fontWeight:700, color:"#633806" }}>La bibliothèque est bien remplie ce soir !</div>
                <div style={{ fontSize:13, color:"#854F0B", marginTop:4 }}>Toutes tes pensées ont trouvé leur place. Bonne nuit 🌙</div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div style={{ maxWidth:"min(560px, 100%)", margin:"0 auto", padding:"0 clamp(8px,3vw,16px)", position:"relative", zIndex:1, boxSizing:"border-box" }}>
          ...
          <AnimatePresence mode="wait">
            {!isOpen && (
              <motion.div key="intro"
                ...
                {[
                  {w:180,h:180,x:"-8%",y:"10%",   c:"#A8E8D8", blur:40, d:0},
                  {w:140,h:140,x:"75%",y:"5%",    c:"#F8C8DC", blur:35, d:1.5},
                  {w:120,h:120,x:"15%",y:"60%",   c:"#C8D8FF", blur:30, d:0.8},
                  {w:100,h:100,x:"72%",y:"55%",   c:"#A8E8D8", blur:28, d:2.2},
                ...
                <motion.div
                  ...
                  <div style={{
                    ...
                    color: isNight ? "#A8E8D8" : "#3A8A78",
                    marginBottom:20,
                  }}>
                    Ta bibliothèque magique t'attend.<br/>
                    <span style={{ color: isNight ? "#F8C8DC" : "#C85A7A", fontWeight:700 }}>
                      Dépose tes pensées
                    </span>
            ...
            {!isOpen ? (
              <motion.button whileTap={{ scale:0.96 }} onClick={handleOpen}
                ...
                style={{ background:"linear-gradient(135deg,#8B6030 0%,#C4903A 50%,#8B6030 100%)",
                  ...
                  color:"#FDF8F1", border:"none", borderRadius:24,
                  padding:"14px 40px", fontSize:15, fontWeight:800, cursor:"pointer", minHeight:44,
              ...
              <motion.button whileTap={{ scale:0.96 }} onClick={handleClose}
                ...
                style={{ background:"#fff", color:"#5A3C1E", border:"2.5px solid #C4A882", borderRadius:24,
                  padding:"11px 28px", fontSize:15, fontWeight:800, cursor:"pointer", minHeight:44,
                  fontFamily:FF.body, display:"inline-flex", alignItems:"center", gap:8,
                  boxShadow:"0 2px 12px rgba(90,60,30,0.18)" }}>
            ...
            {isOpen && total > 0 && (
              <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}
                ...
                <button onClick={handleSleep}
                  style={{ background:"#0B0922", color:"#EEEDFE", border:"none", borderRadius:24,
                    padding:"14px 36px", fontSize:15, fontWeight:800, cursor:"pointer", minHeight:44,
                    fontFamily:FF.body, display:"inline-flex", alignItems:"center", gap:8 }}>
          ...
          <div style={{ textAlign:"center", paddingBottom:32,
            ...
            <button onClick={() => setShowPrivacy(true)}
              style={{ background:"none", border:"none", fontSize:11, color:"#8A7A70",
                cursor:"pointer", minHeight:44, fontFamily:FF.body, textDecoration:"underline" }}>
              Politique de confidentialité
            </button>
            <span style={{ fontSize:11, color:"#D3C9C0" }}>·</span>
            <button onClick={() => setShowCGU(true)}
              style={{ background:"none", border:"none", fontSize:11, color:"#8A7A70",
                cursor:"pointer", minHeight:44, fontFamily:FF.body, textDecoration:"underline" }}>
              Conditions d'utilisation
            </button>
            <span style={{ fontSize:11, color:"#D3C9C0", fontFamily:FF.body }}>
              · v1.0.0
            </span>
          </div>
        </div>
